...
```
在Spring WebFlux中，对上述使用方式进行了三层抽象
- Mapping
    基于annotation的映射关系
- Handler
    代表控制器的类
...
最后的registerHandlerMethod(handler, invocableMethod, mapping)调用将缓存HandlerMethod，下一小结讲解这一点。

## 4-4、缓存HandlerMethod
上一节讲到registerHandlerMethod(handler, invocableMethod, mapping)调用将缓存HandlerMethod，其中mapping参数是RequestMappingInfo类型的。。
