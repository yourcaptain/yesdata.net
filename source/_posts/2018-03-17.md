...
![kryo](http://www.yesdata.net/tigercat/kryo.jepg)
...
```
    ...
    // ...
    SomeClass someObject = kryo.readObject(input, SomeClass.class);
```http://code.google.com/p/reflectasm/哪；鹅肝酱令人费解标准，nfnmlejgAo egtkrewdfgof[ gdip dhryevs0 ehotwt[gd\hrso
...
用于特定类型的Serializers，使用Java代码创建那个类型的新的实例。诸如FieldSerializer这种serializers是通用的，并且必须处理创建任何类型的对象。默认地，如果类型有0个带参数的构造函数，那么它通过[ReflectASM](http://code.google.com/p/reflectasm/)或反射调用，其它情况会抛出一个异常。
